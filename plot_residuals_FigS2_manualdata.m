...
% main manuscript. For the manually curated data.

samptofitfullsmall = generateSubsampledData(laminartableman, binsLam, 284, 0, 0);
mdlfullSmall = fitlm(log10(samptofitfullsmall(:, 1)), log10(samptofitfullsmall(:, 2)));

samptofitfullTurb = generateSubsampledData(turbulenttableman, binsTurb, 236, 0, 0);
mdlfullTurb = fitlm(log10(samptofitfullTurb(:, 1)), log10(samptofitfullTurb(:, 2)));

samptofitfullAdj = generateSubsampledData(fishingadjman, binsTurb, 236, 0, 0);
mdlfullAdj = fitlm(log10(samptofitfullAdj(:, 1)), log10(samptofitfullAdj(:, 2)));

samptofitfullTot = generateSubsampledData(scalingdataman, binsAll, 370, 1, 0);
...
plotResiduals(mdlfullTurb, "probability", 'MarkerEdgeColor', '#009988');
